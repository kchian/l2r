...
from common.models.deeplabv3.deeplab import DeepLab
from common.utils import RecordExperience
import torch.nn.functional as F
from PIL import Image
import common.custom_transforms as tr
...
class SACAgent(AbstractAgent):
    ...
    def __init__(self, env, cfg, args, loggers=tuple(), save_episodes=True, save_batch_size=256, 
        ...
        self.model_delay = args.model_delay
        self.metadata = {}
        ...
        assert self.cfg['use_encoder_type'] in ['vae', 'vae_small', 'resnet', 'deeplab'], \
        ...
        if 'resnet' in self.cfg['use_encoder_type']:
            ...
            self.backbone.load_state_dict(torch.load(self.cfg['vae_small']['vae_chkpt_statedict'],\
                    map_location=DEVICE))
        elif self.cfg['use_encoder_type'] == 'deeplab':
            self.backbone = DeepLab(backbone = self.cfg['deeplab']['backbone'])
            
            checkpoint = torch.load(self.cfg['deeplab']['segm_chkpt_statedict'])
            ...
            self.encoder.load_state_dict(torch.load(self.cfg['vae_small']['vae_chkpt_statedict'],\
                    map_location=DEVICE))
    ...
    def _encode(self, o):
        ...
        if self.cfg['use_encoder_type'] == 'resnet':
            ...
            self.using_speed = 1
        elif self.cfg['use_encoder_type'] == 'deeplab': # TO BE UPDATED
            img = img[0].reshape(720, 960, 3)
            image_keep = torch.tensor(img).permute(2, 0, 1).view(1, 3, 720, 960)
            image_keep = torch.nn.functional.interpolate(image_keep, size = (360, 480))
            image_keep = image_keep.to(DEVICE)
            if (np.max(img) <= 1):
                img = img * 255
            raw_img = Image.fromarray(img).convert('RGB')
            img = self.transform_tr(raw_img).float().to(DEVICE)
            img = img.view(1, 3, 360, 480)
            seg_res = torch.sigmoid(self.backbone(img))[:,1:2,:,:]
     #       sing_c_res = (seg_res.detach().cpu().numpy()[0, 0] * 255).astype(np.uint8)
     #       print(sing_c_res.shape)
     #       seg_img = Image.fromarray(sing_c_res)
     #       seg_img.show()
            
            # img = torch.nn.functional.interpolate(img, size = (480, 360))
            image_keep = image_keep/255
            seg_res = (seg_res > 0.5).float()
            
            
            img_seg = torch.cat([image_keep, seg_res], dim = 1)
            img_seg = F.interpolate(img_seg, size = [144, 144])[:, :, 68:110, :]
            img_embed = self.encoder.encode(img_seg)[0][0]
            speed = torch.tensor((state[4]**2 + state[3]**2 + state[5]**2)**0.5).float().reshape(1, -1).to(DEVICE)
            out = torch.cat([img_embed.unsqueeze(0), speed], dim = -1).squeeze(0) # BS X 513
    ...
    def sac_train(self):
        ...
        for t in range(self.t_start, self.cfg['total_steps']):
            ...
            if (not np.allclose(state2[15:16], state[15:16], atol=3e-1, rtol=0)) | (r!=0):
                ## Do not save actions from the safety controller
                if self.store_from_safe | (self.record['transition_actor'] != 'safepol'): 
